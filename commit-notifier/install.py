...
def create_cronjob(user , repo):
    ...
    print("Ensuring cron service is running...")
    subprocess.run(["sudo", "systemctl" , "enable" , "--now" , "cronie"], check=True)
    
    cron_line = f"*/10 * * * * DISPLAY=1 DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/1000/bus /usr/local/bin/notifier {user} {repo} >> /home/retr0/mew.txt  \n"
